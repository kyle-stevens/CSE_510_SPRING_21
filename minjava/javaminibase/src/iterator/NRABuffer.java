 ...
 */
public class NRABuffer implements GlobalConst {
  ...
  public void updateCurrent(Tuple buf) {
    int temp_curr_page, destination;
    if(t_rd_from_pg == 0) {
      temp_curr_page = rd_curr_page - 1;
      destination = (t_per_pg -1) * t_size;
    } else {
      temp_curr_page = rd_curr_page;
      destination = (t_rd_from_pg - 1) * t_size;
    }

    byte[] copybuf;
    copybuf = buf.getTupleByteArray();
    System.arraycopy(copybuf, 0, _bufs[temp_curr_page], destination, t_size);
    dirty = true;
  }
