...
public class Heapfile implements Filetype, GlobalConst {
	...
	private HFPage populateDataPage(DataPageInfo dpinfop, PageId pid) throws Exception {
		Page page = new Page();
		pinPage(pid, page, true);
		HFPage hfpage = new HFPage(page);
		hfpage.init(pid, page);
		dpinfop.pageId.pid = pid.pid;
		dpinfop.recct = 0;
		dpinfop.availspace = hfpage.available_space();
	...
	public Heapfile(String name, boolean clustered)
			throws HFException, HFBufMgrException, HFDiskMgrException, IOException {
		this(name);
	 ...
	 * @throws IOException
	 */
	public RID insertRecord(byte[] recPtr, int maxCapacity)
	 ...
	 * @throws Exception
	 */
	public RID insertRecord(byte[] recPtr, PageId pid) throws Exception {
		int dpinfoLen = 0;
		int recLen = recPtr.length;
		boolean found;
		RID currentDataPageRid = new RID();
		...
		while (!found) { // Start While01
			...
			if (found == false) {
				...
				if (nextDirPageId.pid != INVALID_PAGE) {
					...
					if (currentDirPage.available_space() >= dpinfo.size) {
						currentDataPage = populateDataPage(dpinfo, pid);
