...
public class ClusteredBtreeIndex extends IndexFile
     ...
     */
    public ClusteredBtreeIndex(String filename)
            throws GetFileEntryException,
            PinPageException,
     ...
     */
    public ClusteredBtreeIndex(String filename, int keytype,
        ...
        //Added in for ClusteredBTree Type
        /* In client option, use this to sort data first by some chosen value, then insert sorted data pairs replacing
        'this' with a ClusteredBtreeIndex object type
        RID rid = new RID();
        float key = 0;
        Tuple temp = null;
        Scan scan = new Scan(new Heapfile("sorting_file"));
        try {
        ...
        while (temp != null) {
            ...
            try {
                this.insert(new RealKey(key), rid);
            } catch (Exception e) {
                e.printStackTrace();
            }
